...
// level Origin integration. The repository and branch are configured by the corresponding top-level attributes. (see below for nested schema)
// Use the installed account-level Origin integration. The repository and branch are configured by the corresponding top-level attributes.
...
// +crossplane:generate:reference:type=github.com/coopnorge/provider-upjet-spacelift/apis/cluster/space/v1alpha1.Space
...
// Spacelift Repos have no branches, and the module always tracks the latest commit. The repo must be in the same space as the module, since the module publishes its source as a version. (see below for nested schema)
// Take the source from a Spacelift repo. The block takes no settings: `repository` is the repo's ID (slug), and `branch` must be `main` - Spacelift Repos have no branches, and the module always tracks the latest commit. The repo must be in the same space as the module, since the module publishes its source as a version.
SpaceliftRepo []SpaceliftRepoInitParameters `json:"spaceliftRepo,omitempty" tf:"spacelift_repo,omitempty"`
...
// Spacelift Repos have no branches, and the module always tracks the latest commit. The repo must be in the same space as the module, since the module publishes its source as a version. (see below for nested schema)
// Take the source from a Spacelift repo. The block takes no settings: `repository` is the repo's ID (slug), and `branch` must be `main` - Spacelift Repos have no branches, and the module always tracks the latest commit. The repo must be in the same space as the module, since the module publishes its source as a version.
SpaceliftRepo []SpaceliftRepoParameters `json:"spaceliftRepo,omitempty" tf:"spacelift_repo,omitempty"`
...
type SpaceliftRepoInitParameters struct {

}


type SpaceliftRepoObservation struct {

}


type SpaceliftRepoParameters struct {
