...
// Spacelift Repos have no branches, and the stack always tracks the latest commit. (see below for nested schema)
// Take the source from a Spacelift repo. The block takes no settings: `repository` is the repo's ID (slug), and `branch` must be `main` - Spacelift Repos have no branches, and the stack always tracks the latest commit.
