...
: ${CRD_ROOT_GROUP:=$(read -r -p "CRD rootGroup (ex. crossplane.io): " CRD_ROOT_GROUP; echo -n "${CRD_ROOT_GROUP}")}
...
# shellcheck disable=SC2086
git grep -l "ujconfig\.WithRootGroup(\"${PROVIDER_NAME_LOWER}.upbound\.io\")" -- "config/provider.go" | xargs sed -i.bak "s|ujconfig.WithRootGroup(\"${PROVIDER_NAME_LOWER}.crossplane.io\")|ujconfig.WithRootGroup(\"${CRD_ROOT_GROUP}\")|g"
